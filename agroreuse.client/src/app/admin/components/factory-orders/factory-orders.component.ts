...
@Component({
  ...
  styleUrls: ['./factory-orders.component.css']
})
export class FactoryOrdersComponent implements OnInit {
  ...
  loadOrders(): void {
    this.loading = true;
    this.error = '';
    this.orderService.getFactoryOrders().subscribe({
      next: (data) => {
        this.orders = data;
        this.applyFilters();
        this.loading = false;
      ...
      error: (err: Error) => {
        this.error = 'فشل تحميل طلبات المصانع';
        this.loading = false;
        console.error('Error loading factory orders:', err);
      }
    });
  }
